...
static NTSTATUS smb1390_read(SPB_CONTEXT* regmap, u16 addr, u8* val)
{
	NTSTATUS rc;
	...
	rc = SpbReadDataSynchronously(regmap, cmd, 2, &val, 1, 0);
	if (rc)
		DbgPrint("Couldn't read 0x%04x\n", addr);

	return rc;
}

static NTSTATUS smb1390_update_bits(SPB_CONTEXT* regmap,u16 reg, u8 mask, u8 val ) {
	NTSTATUS status;
	u8 temp_val, data;
	status = smb1390_read(regmap, reg, &data);
	...
	cmd[1] = reg & 0xff;
	cmd[0] = (reg >> 8) & 0xff;
	status = SpbWriteDataSynchronously(regmap, cmd, 2, &temp_val, 1);
	return status;
}
static NTSTATUS smb1390_masked_write(SPB_CONTEXT* regmap, u16 reg, u8 mask,
	u8 val)
{
	NTSTATUS rc;

	DbgPrint("Writing 0x%02x to 0x%04x with mask 0x%02x\n", val, reg, mask);
	rc = smb1390_update_bits(regmap, reg, mask, val);
	if (rc < 0)
		DbgPrint("Couldn't write 0x%02x to 0x%04x with mask 0x%02x\n",
			val, reg, mask);

	return rc;
}

NTSTATUS  charger_init(struct device_node* dev_node) {
	NTSTATUS rc;
	u8 val;
	SPB_CONTEXT* chip = &dev_node->I2CContext;
