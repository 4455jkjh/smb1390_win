...
static u8 qpnp_read_byte(SPB_CONTEXT* regmap, u16 addr)
{
	NTSTATUS rc;
	u8 val;
	u8 cmd[2];
...
NTSTATUS revid_init(struct device_node* dev_node) {
	...
	int fab_id, tp_rev;
	struct pmic_revid_data data = dev_node->data;
	SPB_CONTEXT* regmap = &dev_node->I2CContext;
	u16 base = revid_base;
