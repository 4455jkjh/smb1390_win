...

{
    ...
    SimBattPrepareHardware(FxDevice);
    Device = GetDeviceExtension(FxDevice);
    Status = STATUS_INSUFFICIENT_RESOURCES; resourceCount = WdfCmResourceListGetCount(ResourcesTranslated);
    for (i = 0; i < resourceCount; i++)
    {
        UCHAR Class;
        UCHAR Type;
        pDescriptor = WdfCmResourceListGetDescriptor(ResourcesTranslated, i);
        ...
        case  CmResourceTypeConnection:
                ...
                Type == CM_RESOURCE_CONNECTION_TYPE_SERIAL_I2C)
            {
                ...
                {
                    Status = STATUS_SUCCESS;
                    Device->I2CContext.I2cResHubId.LowPart = pDescriptor->u.Connection.IdLowPart;
                    Device->I2CContext.I2cResHubId.HighPart = pDescriptor->u.Connection.IdHighPart;
    ...
    if (fSpbResourceFound == FALSE)
    {
        ...
        Status = SpbTargetInitialize(FxDevice, &Device->I2CContext);
    }
    Device->addr[0] = 0x10;
    Device->addr[1] = INT_SET_TYPE_OFFSET;
    SpbReadDataSynchronously(&Device->I2CContext, Device->addr, 2, Device->cached, IRQ_MAX_REGS, FALSE);
    memcpy(Device->synced, Device->cached,
        IRQ_MAX_REGS * sizeof(*Device->synced));
    Status = revid_init(Device);
    if (Status == 0)
        Status = charger_init(Device);
    DebugExitStatus(Status);
    return Status;
}
